...
<body class="bg-white-100 ">
    ...
    <div class="wrapper">
        ...
        <div class="custom-form-container">
            <form action="../database/update_product.php" method="POST" enctype="multipart/form-data">
                <input type="hidden" name="id_product" value="<?php echo $product_row['product_id']; ?>">

                <div class="mb-4">
                    <label class="custom-label" for="product_name">Tên sản phẩm:</label>
                    <input type="text" name="product_name" id="product_name"
                        value="<?php echo htmlspecialchars($product_row['product_name']); ?>" required>
                </div>

                <div class="mb-4">
                    <label class="custom-label" for="description">Mô tả sản phẩm:</label>
                    <textarea name="description" id="description" rows="4"
                        required><?php echo htmlspecialchars($product_row['description']); ?></textarea>
                </div>

                <div class="mb-4">
                    <label class="custom-label">Hình ảnh sản phẩm:</label>
                ...
                <div style="margin-bottom: 1rem;">
                    <label for="category"
                        style="display: block; margin-bottom: 0.5rem; font-weight: 600; color: #333;">Danh mục</label>
                    <select id="category" name="category_id" required
                        style="width: 100%; padding: 0.375rem 0.75rem; font-size: 1rem; line-height: 1.5; color: #495057; background-color: #fff; border: 1px solid #ced4da; border-radius: 0.25rem; transition: border-color 0.15s ease-in-out, box-shadow 0.15s ease-in-out;">
                        <option value="">Chọn danh mục</option>
                        <option value="1" <?php echo ($product_row['category_id'] == 1) ? 'selected' : ''; ?>>Cửa kính
                        </option>
                        <option value="2" <?php echo ($product_row['category_id'] == 2) ? 'selected' : ''; ?>>Cửa nhôm
                        </option>
                        <option value="3" <?php echo ($product_row['category_id'] == 3) ? 'selected' : ''; ?>>Tay cầm cửa
                        </option>
                    </select>
                </div>

                <div style="margin-bottom: 1rem;">
                    <label style="display: block; margin-bottom: 0.5rem; font-weight: 600; color: #333;">Trạng
                        thái</label>
                    <div style="display: flex; align-items: center; margin-bottom: 0.5rem;">
                        <input type="radio" name="status" id="show" value="1" <?php echo ($product_row['status'] == 1) ? 'checked' : ''; ?> style="margin-right: 0.5rem; width: 1rem; height: 1rem;">
                        <label for="show" style="margin: 0; font-size: 1rem; color: #333;">Hiển thị</label>
                    </div>
                    <div style="display: flex; align-items: center; margin-bottom: 0.5rem;">
                        <input type="radio" name="status" id="hide" value="0" <?php echo ($product_row['status'] == 0) ? 'checked' : ''; ?> style="margin-right: 0.5rem; width: 1rem; height: 1rem;">
                        <label for="hide" style="margin: 0; font-size: 1rem; color: #333;">Ẩn</label>
                    </div>
                </div>
        ...
        <div class="custom-form-container">
            <fieldset class="border-4 p-5">
                <form action="/database/save_content.php" method="POST" enctype="multipart/form-data">
                    ...
                    <div id="blocksContainer">
                        ...
                        while ($row = $result->fetch_assoc()) {
                            $contentId = $row['id_product_description'];
