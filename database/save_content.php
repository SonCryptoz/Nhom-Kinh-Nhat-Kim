...
if (isset($_POST['content_id']) && isset($_POST['delete_existing'])) {
    ...
    foreach ($content_ids as $index => $content_id) {
        if ($delete_flags[$index] == "1") {
            $sql = "DELETE FROM product_description WHERE id = ? AND product_id = ?";
...
if (isset($_POST['content_id']) && isset($_POST['delete_existing'])) {
    ...
    foreach ($content_ids as $index => $content_id) {
        ...
        $sql = "UPDATE product_description SET content = ?, position = ? WHERE id_product_description = ? AND product_id = ?";
        $stmt = $conn->prepare($sql);
...
if (isset($_POST['new_content_type'])) {
    ...
    foreach ($new_types as $i => $new_type) {
        ...
        $sql = "INSERT INTO product_description (product_id, type, content, position) VALUES (?, ?, ?, ?)";
        $stmt = $conn->prepare($sql);
...
$conn->close();
header("Location: ../admin/update-product-admin.php?id=" . $id_product);
